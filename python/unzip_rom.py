...
def upload_small_files(bucket, target_folder, local_zip_path, zip_name, small_files):
...
def split_large_file(bucket, target_folder, local_zip_path, zip_name, member_name, has_header):
    clean_name = member_name.replace("\\", "/").strip("/")
...
def process_zip_file(bucket, target_folder, zip_key, has_header):
    ...
    try:
        ...
        if small_files:
            upload_small_files(bucket, target_folder, local_zip_path, zip_name, small_files)

        if large_files:
            log.info("Phase 2: processing large files one by one")
            for m in large_files:
                log.info("Splitting file: %s", m.filename)
                split_large_file(bucket, target_folder, local_zip_path, zip_name, m.filename, has_header)
