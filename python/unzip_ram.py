...
def upload_count_file(s3, bucket, key, headers, rows):
...
def split_large_file(
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        source_bytes = zf.read(member_name)
...
def process_zip_file(
    ...
    if small_files:
        small_results = upload_small_files(
            s3=s3,
            bucket=bucket,
    ...
    count_location = normalize_prefix(count_location)

    source_count_key = f"{count_location}/{zip_name}_source_count_{ts}.csv"
    processed_count_key = f"{count_location}/{zip_name}_processed_count_{ts}.csv"

    upload_count_file(
        s3=s3,
        bucket=bucket,
        key=source_count_key,
        headers=[
            "zip_file_name",
        ...
        rows=source_count_rows
    )

    upload_count_file(
        s3=s3,
        bucket=bucket,
        key=processed_count_key,
        headers=[
            "zip_file_name",
            "source_full_csv_file_name",
