    ...
    def _load_data_dictionary(self, path):
        ...
        try:
            ...
            if isinstance(path, str) and path.startswith("s3://"):
                ...
                print(f"Loading data dictionary from S3: s3://{bucket}/{key}")
                s3 = boto3.client("s3")
                obj = s3.get_object(Bucket=bucket, Key=key)
                df_dict = pd.read_csv(io.BytesIO(obj["Body"].read()))
            ...
            # Build sensitive_fields from df_dict
            sensitive_fields = {}
            for _, row in df_dict.iterrows():
                field_name = str(row['field_name']).lower().strip()
                field_config = {"type": str(row['type']).lower().strip()}
...
def load_params(param_file_path):
    ...
    if param_file_path.startswith("s3://"):
        ...
        print(f"Loaded parameter file from S3: s3://{bucket}/{key}")
    else:
