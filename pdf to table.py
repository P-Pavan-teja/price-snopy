import re
import tabula
import pdfplumber
import pandas as pd

PDF_PATH = "databases/Databases.pdf"
START_PAGE = 1
END_PAGE   = 300   # set your real internal range

# Extract only Abc.bac.dbc from 'Abc.bac.dbc table' or '"Abc.bac.dbc table"'
DB_RE = re.compile(r'(?i)\b([A-Za-z_][\w$]*\.[A-Za-z_][\w$]*\.[A-Za-z_][\w$]*)\b')

def tabula_json_to_df(tjson: dict) -> pd.DataFrame:
    data = tjson.get("data") or []
    rows = [[cell.get("text", "") if isinstance(cell, dict) else "" for cell in row] for row in data]
    df = pd.DataFrame(rows)
    df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
    df = df.replace(r"^\s*$", pd.NA, regex=True).dropna(how="all").reset_index(drop=True)
    return df

def extract_lines_with_y(plumber_page, y_tol=3):
    words = plumber_page.extract_words()
    if not words:
        return []
    words = sorted(words, key=lambda w: (w["top"], w["x0"]))

    lines = []
    cur, cur_y = [], None
    for w in words:
        y = float(w["top"])
        if cur_y is None or abs(y - cur_y) <= y_tol:
            cur.append(w)
            cur_y = y if cur_y is None else (cur_y + y) / 2
        else:
            lines.append({"top": cur_y, "text": " ".join(x["text"] for x in cur)})
            cur, cur_y = [w], y
    if cur:
        lines.append({"top": cur_y, "text": " ".join(x["text"] for x in cur)})
    return lines

def find_nearest_label_above(lines, table_top, max_scan=650):
    if table_top is None:
        return None
    best_y = None
    best_label = None
    for ln in lines:
        y = float(ln["top"])
        if y < table_top and (table_top - y) <= max_scan:
            m = DB_RE.search(ln["text"])
            if m:
                if best_y is None or y > best_y:
                    best_y = y
                    best_label = m.group(1)
    return best_label

def find_any_label_on_page(lines):
    """
    If label is not near the table (like it is above an image or at top),
    grab the last label on the page.
    """
    last = None
    for ln in sorted(lines, key=lambda x: x["top"]):
        m = DB_RE.search(ln["text"])
        if m:
            last = m.group(1)
    return last

def normalize_3col(df):
    # enforce 3 columns and names
    if df.shape[1] > 3:
        df = df.iloc[:, :3]
    if df.shape[1] != 3:
        return None
    df.columns = ["column_name", "description", "comments"]
    return df

def drop_repeated_header_rows(df):
    header_vals = [c.strip().lower() for c in df.columns]
    def is_header_row(row):
        vals = [str(x).strip().lower() for x in row.tolist()]
        return vals == header_vals
    return df[~df.apply(is_header_row, axis=1)].reset_index(drop=True)

# ---------------------------------------------------------
# Main: carry-forward label + continuation stitching
# ---------------------------------------------------------
all_parts = []
last_label_global = ""   # carry across pages (solves your issue #1 and #2)

with pdfplumber.open(PDF_PATH) as pdf:
    total_pages = len(pdf.pages)
    end = min(END_PAGE, total_pages)

    for page_num in range(START_PAGE, end + 1):
        # Read tables on page
        try:
            tables_json = tabula.read_pdf(
                PDF_PATH,
                pages=str(page_num),
                multiple_tables=True,
                guess=True,
                silent=True,
                output_format="json"
            )
        except Exception as e:
            print(f"Tabula failed on page {page_num}: {e}")
            continue

        # Extract page lines
        lines = extract_lines_with_y(pdf.pages[page_num - 1], y_tol=3)

        # Update global label if page contains any label anywhere
        page_label_anywhere = find_any_label_on_page(lines)
        if page_label_anywhere:
            last_label_global = page_label_anywhere

        if not tables_json:
            continue

        # For each table on page
        for t in tables_json:
            # table bbox top
            top_val = t.get("top", None)
            try:
                table_top = float(top_val) if top_val is not None else None
            except:
                table_top = None

            # Try: nearest label above table
            label = find_nearest_label_above(lines, table_top, max_scan=650)

            # Fallback 1: any label on same page
            if not label:
                label = page_label_anywhere

            # Fallback 2: carry-forward from previous pages (KEY FIX)
            if not label:
                label = last_label_global

            df = tabula_json_to_df(t)
            if df.empty:
                continue

            df = normalize_3col(df)
            if df is None:
                continue

            df = drop_repeated_header_rows(df)

            df.insert(0, "table_name", label or "")
            df.insert(1, "__page__", page_num)

            all_parts.append(df)

# Combine all extracted chunks
combined = pd.concat(all_parts, ignore_index=True) if all_parts else pd.DataFrame()
display(combined)

# Save
# combined.to_csv("tables_with_table_name.csv", index=False)
